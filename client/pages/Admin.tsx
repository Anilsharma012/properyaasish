...
export default function Admin() {
  ...
  const loadMockData = () => {
    ...
    setUsers([
      { _id: "1", name: "John Doe", email: "john@example.com", userType: "buyer", status: "active" },
      { _id: "2", name: "Jane Smith", email: "jane@example.com", userType: "seller", status: "active" },
      { _id: "3", name: "Mike Johnson", email: "mike@example.com", userType: "agent", status: "active" }
    ]);

    setProperties([
      { _id: "1", title: "Modern Apartment", location: { city: "Mumbai" }, price: 5000000, status: "active" },
      { _id: "2", title: "Villa in Suburbs", location: { city: "Delhi" }, price: 8000000, status: "active" },
      { _id: "3", title: "Commercial Space", location: { city: "Bangalore" }, price: 12000000, status: "pending" }
    ]);
  ...
  useEffect(() => {
    ...
    const testConnectivity = async () => {
      try {
        // Try a simple fetch with a very short timeout
        ...
        console.log("Connectivity test failed, enabling force offline mode");
    ...
    testConnectivity();
  }, [isAuthenticated, user, token, authLoading, forceOfflineMode, skipDataLoading]);
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2">
            <div className={`w-3 h-3 rounded-full ${
              forceOfflineMode ? 'bg-blue-500' :
              (offlineMode || apiErrors.length > 0 ? 'bg-yellow-500' : 'bg-green-500')
            }`}></div>
            <span className="text-sm text-gray-600">
              {forceOfflineMode ? 'Force Offline (Mock Data)' :
               offlineMode ? 'Offline Mode' :
               apiErrors.length > 0 ? 'Limited Connectivity' : 'Online'}
            </span>
          </div>
        </div>
        <div className="flex items-center space-x-2">
      ...
      {apiErrors.length > 0 && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
          <div className="flex items-center space-x-2 mb-2">
            ...
            <p className="text-yellow-700 font-medium">Some services are unavailable</p>
          </div>
      ...
      {loading ? (
        <div className="text-center py-8">
          ...
          <div className="mt-6">
            ...
            <p className="text-xs text-gray-500 mt-2">Use this if loading takes too long</p>
          </div>
        </div>
      ) : offlineMode || apiErrors.length > 2 ? (
        <div className="text-center py-12">
          <AlertTriangle className="h-16 w-16 text-yellow-500 mx-auto mb-4" />
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Admin Dashboard - Safe Mode</h2>
          <p className="text-gray-600 mb-6">Some services are temporarily unavailable. You can still access admin functions using the sidebar menu.</p>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500">Add About Us, Privacy Policy, etc.</p>
                </div>
                <QuickCreatePage />
              </div>
              {[
    ...
    try {
      ...
      return (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-6">There was an error loading this admin section.</p>
          <Button
